            ...
            <div class="ml-1">
                <v-select-user placement="right-0" :users="users" v-model="form.user_uuid" @input="updateTask()">
                    <template v-slot:button="{ selected: user }">
                        <a v-if="user" href="#"
                            class="ml-1 flex items-center justify-center rounded-full overflow-hidden text-gray-400 border border-transparent w-6 h-6 overflow-hidden hover:text-gray-500 hover:bg-gray-50">
                            <img class="avatar avatar-xs" :src="user.avatar_url" :alt="user.name">
                        </a>

                        <a v-else href="#"
                            class="ml-1 flex items-center justify-center rounded-full overflow-hidden text-gray-400 border w-6 h-6 overflow-hidden hover:text-gray-500 hover:bg-gray-50">
                            <svg viewBox="0 0 20 20" fill="currentColor" class="w-3 h-3">
                                <path
