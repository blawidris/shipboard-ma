<template>
    <div class="flex flex-col">
        <div class="mb-6">
            <div class="mt-1 flex rounded-md shadow-sm">
                <div class="relative flex-grow focus-within:z-10">
                    <input class="form-input rounded-r-none" :placeholder="$trans('placeholders.enter-new-task')"
                        v-model="newSubTask.content" @keyup.enter="addTask()" />
                </div>

                <button @click="addTask()" :disabled="!newSubTask.content"
        ...
        <div class="flex flex-col mb-4" v-if="incompletedTasks.length > 0">
            ...
            <v-sub-task :key="task.uuid" :users="users" :task="task" :project-uuid="projectUuid"
                v-for="task in incompletedTasks" @updated="getSubtasks()" />
        </div>

        <div class="flex flex-col mb-6 border-b border-gray-400" v-if="completedTasks.length > 0">
            <div class="pb-2 border-b border-gray-200 mb-1">
                <h4 class="font-medium text-lg">{{ $trans('labels.completed') }} ({{ completedTasks.length }})</h4>
            </div>

            <v-sub-task :key="task.uuid" :users="users" :task="task" :project-uuid="projectUuid"
                v-for="task in completedTasks" @updated="getSubtasks()" />

        </div>
        <div class="mb-6">
            <div class="border-t border-gray-200 py-4">
                <h4 class="font-medium text-lg">{{ $trans('labels.comments') }}</h4>
            </div>
            <v-tab-comments :task-uuid="taskUuid" :project-uuid="projectUuid" @new-comment="onNewCommentSubmitted" />
        </div>
    </div>
</template>
