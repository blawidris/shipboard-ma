                    ...
                    <div class="px-4 py-4 flex items-center sm:px-6">
                        <div class="min-w-0 flex-1 md:grid md:grid-cols-8 sm:flex sm:justify-between sm:items-center">
                            <div class="flex items-center md:col-span-4">
                                <div class="pr-2 sm:pr-6">
                                    <input type="checkbox" class="form-checkbox w-6 h-6 rounded-full text-green-400"
                                        v-model="task.is_completed" @change="updateTask(task)">
                                </div>

                                <a @click.prevent="showTaskModal(task)" href="#"
                                    class="text-sm leading-5 truncate hover:underline">
                                    {{ task.content }}
                                </a>
                            </div>
                            ...
                            <div class="mt-1 sm:mt-0 md:col-span-2">
                                <div>
                                    <div class="flex items-center text-sm leading-5">
                                        <a :href="route('app:projects.show', { project: task.project.uuid })"
                                            ...
                                            <div class="w-3 h-3 overflow-hidden rounded-full mr-2.5 hidden sm:inline-block">
                                                <div class="w-full h-full" :class="['bg-' + task.project.color + '-400']">
                                                </div>
                                            </div>
                                        ...
                                        <v-dropdown placement="left-0">
                                            <template v-slot:button>
                                                <a @click.prevent="" href="#"
                                                    ...
                                                    <svg class="w-4 h-4 mr-1.5 fill-current"
                                            ...
                                            <template v-slot:content>
                                                <v-input-date v-model="task.due_date" @input="updateTask(task)" inline />
                                            </template>
                                        </v-dropdown>
                                    </div>
                                </div>
                            ...
                            <div
                                ...
                                <v-select-user v-model="task.user_uuid" placement="right-0" :users="users"
                                    @input="updateTask(task)">
                                    <template v-slot:button="{ selected: user }">
                                        <a @click.prevent="" v-if="user" href="#"
    ...
    methods: {
        showTaskModal(task) {
            this.$modal(VModalTask, {
                ...
                // isNotApplicable: task.is_not_applicable
            })
        },
