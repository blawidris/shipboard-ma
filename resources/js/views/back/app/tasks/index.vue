<template>
    <v-app-default-layout :pageTitle="$trans('headings.my-tasks')">
        <template v-slot:header>
            <!-- <h1 class="text-2xl font-semibold text-gray-900">{{ $trans('headings.my-tasks') }}</h1> -->
            <v-nav-header :pageTitle="$trans('headings.my-tasks')"></v-nav-header>
        </template>

        <template>

            <v-breadcrumb-layout class="my-5 container  mx-auto px-9 ">
                <v-breadcrumb-item :lastItem="true" :title="$trans('headings.my-tasks')" />
            </v-breadcrumb-layout>

            <div class="container mx-auto w-full flex flex-col flex-1 px-4 sm:px-6 md:px-8 xl:px-8 py-4">
                ...
                <div class="flex flex-col flex-col-reverse md:flex-row mx-0 sm:-mx-6">
                    ...
                    <div class="flex flex-col flex-1 px-0 sm:px-6" v-else>
                        <v-tasks-list :title="$trans('headings.today')" :tasks="todayTasks" :users="users"
                            :show="$request.params() == '' || $request.query('filter') == 'today' || $request.has('project')" />

                        <v-tasks-list :title="$trans('headings.upcoming')" :tasks="upcomingTasks" :users="users"
                            :show="$request.params() == '' || $request.query('filter') == 'upcoming' || $request.has('project')" />

                        <v-tasks-list :title="$trans('headings.overdue')" :tasks="overdueTasks" :users="users"
                            :show="$request.params() == '' || $request.query('filter') == 'overdue' || $request.has('project')" />

                        <v-tasks-list :title="$trans('headings.no-overdue')" :tasks="noOverdueTasks" :users="users"
                            :show="$request.params() == '' || $request.query('filter') == 'no_overdue' || $request.has('project')" />

                        <v-tasks-list :title="$trans('headings.completed')" :tasks="completedTasks" :users="users"
                    ...
                    <div class="flex flex-col flex-shrink-0 w-full px-0 sm:w-64 sm:px-6 mb-6">
                        <div class="mb-6">
                            <nav>
                                ...
                                <inertia-link :class="{ 'bg-white font-medium': $request.query('filter') == 'today' }"
                                ...
                                <inertia-link :class="{ 'bg-white font-medium': $request.query('filter') == 'upcoming' }"
                                ...
                                <inertia-link :class="{ 'bg-white font-medium': $request.query('filter') == 'overdue' }"
                                ...
                                <inertia-link :class="{ 'bg-white font-medium': $request.query('filter') == 'no_overdue' }"
                                ...
                                <inertia-link :class="{ 'bg-white font-medium': $request.query('filter') == 'completed' }"
