<template>
    <v-app-default-layout :pageTitle="$trans('headings.projects')">
        <template v-slot:header>
            <v-nav-header :pageTitle="$trans('headings.projects')"></v-nav-header>
        </template>
                        ...
                        <div class="mt-1 flex rounded-md shadow-sm">
                            <div class="relative flex-grow focus-within:z-10">
                                ...
                                <input
                                    ...
                                    :placeholder="$trans('placeholders.search')" v-model="filtersForm.search" />
                            </div>
                                ...
                                <template v-slot:content>
                                    <div class="dropdown-menu p-4">
                                        ...
                                        <div class="form-group">
                                            <label class="form-label">{{ $trans('labels.timeline') }}</label>
                                            <!-- date card -->
                                            <v-input-date id="project-timeline" mode="range"
                                                placeholder="YYYY-MM-DD to YYYY-MM-DD" v-model="filtersForm.timeline" />

                                        </div>

                                        <!-- <div class="form-group">
                                            <label class="form-label">{{ $trans('labels.due_date') }}</label>
                                            <v-input-date inline v-model="filtersForm.end_date" @input="" />
                                        </div> -->
                                    </div>
                                </template>
                            </v-dropdown>
                                ...
                                <div class="px-6 py-4 flex items-center">
                                    <div class="min-w-0 flex-1 md:grid md:grid-cols-8 md:gap-4">
                                        ...
                                        <div class="mt-2 md:col-span-2 sm:mt-0">
                                            <div>
                                                <div class="flex items-center text-sm leading-5 text-gray-500">
                                                    <svg class="flex-shrink-0 mr-1.5 h-4 w-4 text-gray-400 fill-current"
                                                        viewBox="0 0 20 20">
                                                        <path
                                                    ...
                                                    <span class="font-medium">{{ project.pending_tasks }}</span>&nbsp;{{
                                                        $trans('labels.tasks-left') }}
                                                </div>
                                                ...
                                                <div class="mt-2 flex items-center text-sm leading-5 text-gray-500">
                                                    ...
                                                    <span v-if="project.days_left && project.days_left > 0">
                                                        <span class="font-medium">{{ project.days_left }}</span> {{
                                                            $trans('labels.days-left') }}
                                                    </span>
                                                    ...
                                                    <span v-else-if="project.days_left && project.days_left < 0"
                                                        class="text-red-700">
                                                        <span class="font-medium">{{ project.days_left * -1 }}</span> {{
                                                            $trans('labels.days-overdue') }}
                                                    </span>
                                        ...
                                        <div class="mt-2 flex items-center md:col-span-1 sm:mt-0">
                                            ...
                                            <span class="badge badge-red" v-else-if="project.status === 'overdue'">{{
                                                $trans('labels.overdue') }}</span>
                                            <span class="badge badge-green"
                                                v-else-if="project.status === 'completed' && project.approved === '1'">{{
                                                    $trans('labels.completed') }}</span>
                                            <span class="badge badge-yellow"
                                                v-else-if="project.status === 'completed' && project.approved !== '1'">{{
                                                    $trans('labels.pending') }}</span>
                                            <span class="badge" v-else-if="project.status === 'archived'">{{
                                                $trans('labels.organization') }}</span>
                                        </div>

                                        <div class="mt-4 flex-shrink-0 flex items-center sm:mt-0"
                                            v-if="project.team_members.length">
                                            <div class="avatar-group">
                                                <img v-for="user in project.team_members" class="avatar avatar-xs"
    ...
    data() {
        return {
            filtersForm: {
                ...
                timeline: this.filters.timeline
            }
        }
    },
